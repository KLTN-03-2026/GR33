            ...
            <div class="d-flex align-items-center gap-2">
                <div v-if="$authSV.user.trang_thai != 1" class="text-end me-2">
                    <span class="badge bg-warning text-dark px-3 py-2 rounded-pill shadow-sm" style="font-size: 0.8rem">
                        ...
                        {{ $authSV.user.trang_thai == 2 ? 'Tạm dừng thêm chứng chỉ (Đang Bảo lưu)' : ($authSV.user.trang_thai == 3 ? 'Chức năng thêm chứng chỉ đã đóng (Tốt nghiệp)' : 'Tài khoản đang bị đình chỉ') }}
                    </span>
                </div>
                <button class="btn-pink shadow-sm" @click="moModalThem" :disabled="$authSV.user.trang_thai != 1"
                    :title="$authSV.user.trang_thai != 1 ? 'Bạn không thể thêm chứng chỉ ở trạng thái hiện tại' : 'Thêm chứng chỉ'">
