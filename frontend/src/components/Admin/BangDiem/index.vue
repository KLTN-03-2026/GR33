            ...
            <tr v-else v-for="(item, index) in danhSachPhanTrang" :key="item.id">
              ...
              <td class="text-end">
                <div class="d-flex justify-content-end gap-2">
                  ...
                  <button v-if="!item.is_locked && $hasPermission(43)" class="btn btn-action shadow-sm" @click="moModalSua(item)" title="Sửa">
                  ...
                  <button v-if="item.trang_thai !== 1 && !item.is_locked && $hasPermission(44)" class="btn btn-action shadow-sm" @click="xuLyXoa(item)" title="Xóa">
                    <i class="bi bi-trash-fill text-danger"></i>
                  </button>
                  <button v-else-if="item.trang_thai === 1 || item.is_locked" class="btn btn-action shadow-sm opacity-50 cursor-not-allowed" disabled title="Không thể xóa điểm đã đúc NFT hoặc đã khóa">
          ...
          <div class="modal-body p-4">
            <form @submit.prevent="xuLyLuu">
              <div class="row g-3">
                ...
                <div class="col-md-6">
                  <label class="form-label fw-700 small text-uppercase opacity-75">Lớp học (Đang diễn ra)</label>
