<?php

namespace App\Http\Requests;

use Illuminate\Foundation\Http\FormRequest;

class StoreBangDiemRequest extends FormRequest
{
    public function authorize(): bool
    {
        return true;
    }

    public function rules(): array
    {
        return [
            'sinh_vien_id'   => [
                'required', 
                'exists:sinh_viens,id',
                function ($attribute, $value, $fail) {
                    $exists = \App\Models\BangDiem::where('sinh_vien_id', $value)
                                ->where('lop_hoc_id', $this->lop_hoc_id)
                                ->exists();
                    if ($exists) {
                        $fail('Sinh viên này đã có tên trong danh sách lớp học này rồi.');
                    }
                }
            ],
            'lop_hoc_id'     => ['required', 'exists:lop_hocs,id'],
            'diem_qua_trinh' => ['nullable', 'numeric', 'min:0', 'max:10'],
            'diem_cuoi_ky'   => ['nullable', 'numeric', 'min:0', 'max:10'],
        ];
    }

    public function messages(): array
    {
        return [
            'sinh_vien_id.required'   => 'Sinh viên là bắt buộc.',
            'sinh_vien_id.exists'     => 'Sinh viên không tồn tại.',
            'lop_hoc_id.required'     => 'Lớp học là bắt buộc.',
            'lop_hoc_id.exists'       => 'Lớp học không tồn tại.',
            'diem_qua_trinh.numeric'  => 'Điểm quá trình phải là số.',
            'diem_qua_trinh.min'      => 'Điểm quá trình tối thiểu là 0.',
            'diem_qua_trinh.max'      => 'Điểm quá trình tối đa là 10.',
            'diem_cuoi_ky.numeric'    => 'Điểm cuối kỳ phải là số.',
            'diem_cuoi_ky.min'        => 'Điểm cuối kỳ tối thiểu là 0.',
            'diem_cuoi_ky.max'        => 'Điểm cuối kỳ tối đa là 10.',
        ];
    }
}
